...
test('prepareChatRequest keeps drafts separate and preserves mobile-only requested session ids', () => {
  ...
  assert.equal(mobileOnly.selectedSessionId, null);
  assert.equal(mobileOnly.draftSessionId, 'thread-mobile-only');
...
test('projectlessThreadWorkingDirectory creates dated slug directories', async () => {
  ...
  assert.equal(cwd, path.join(path.resolve('/tmp/codex-projectless'), '2026-05-08', 'hello-world-21i3v9'));
