...
// TOML basic strings escape backslashes, so a Windows path lands in
// config.toml as `[projects."\\?\\D:\\project\\x"]` (and device paths as
// `\\\\?\\`). Reading the key verbatim left the doubled separators in place,
// which path.resolve later turned into `D:\?\D:\project\x` phantom projects.
// Literal strings ('single quoted') carry no escapes and must stay verbatim.
function unescapeTomlBasicString(value) {
  return String(value || '').replace(/\\(["\\bfnrt])/g, (_, char) => {
    switch (char) {
      case 'b': return '\b';
      case 'f': return '\f';
      case 'n': return '\n';
...
export async function readCodexWorkspaceState() {
  try {
    ...
    for (const root of orderedRoots) {
      ...
      // Newer Codex desktop builds mix thread ids ("local-<hash>", bare
      // UUIDs) into project-order alongside real workspace roots. Resolving
      // those against the server cwd would materialize phantom empty projects.
...
export async function readCodexConfig() {
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    if (projectMatch) {
      currentProject = projectMatch[1] !== undefined
        ? projectMatch[1]
        : unescapeTomlBasicString(projectMatch[2]);
