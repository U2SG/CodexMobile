...
function defaultProjectlessWorkspaceRoot() {
...
export function normalizeSelectedSkills(value, availableSkills = []) {
  ...
  for (const skill of availableSkills) {
    if (skill?.path) byPath.set(String(skill.path), skill);
    if (skill?.name) byName.set(String(skill.name), skill);
  }
...
export function prepareChatRequest(body = {}, {
  ...
  const codexMessageBase = String(body.codexMessage || '').trim() || message || '请查看附件。';
  const codexMessage = withFileMentionReferences(
    withAttachmentReferences(codexMessageBase, attachments),
    fileMentions
  );
