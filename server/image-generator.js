...
const GENERATE_PATTERNS = [
  /(?:生图|画图|出图|生成(?:一张|张)?图|做(?:一张|张)?图|来(?:一张|张)?图)/i,
  /(?:生成|画|绘制|制作|设计|出图|创建|做一张|做张|来一张|来张).*(?:图片|图像|照片|海报|宣传图|壁纸|头像|插画|封面|logo)/i,
  /(?:图片|图像|照片|海报|宣传图|壁纸|头像|插画|封面|logo).*(?:生成|画|绘制|制作|设计|出图|创建|做)/i,
...
const HIGH_CONFIDENCE_GENERATE_PATTERNS = [
  /(?:生成|画|绘制|制作|创建|做|来)(?:一张|张|个)?(?:图片|图像|照片|海报|壁纸|头像|插画)/i,
  /(?:图片|图像|照片|海报|壁纸|头像|插画).*(?:生成|画|绘制|制作|创建|做)/i,
  /\b(?:generate|create|draw|make)\s+(?:an?\s+)?(?:image|photo|picture|poster|wallpaper|avatar)\b/i,
  /\b(?:generate|create|draw|make)\b.*\b(?:image|photo|picture|poster|wallpaper|avatar)\b/i
];

const MEDIUM_CONFIDENCE_GENERATE_PATTERNS = [
  /(?:设计|制作|生成|创建|画|绘制).*(?:logo|标志|封面|视觉|主图)/i,
  /(?:logo|标志|封面|视觉|主图).*(?:设计|制作|生成|创建|画|绘制)/i,
  /\b(?:design|make|create|generate|draw)\b.*\b(?:logo|cover|visual|banner)\b/i
];
...
export function analyzeImageIntent(message, attachments = []) {
  const text = String(message || '').trim();
  const imageAttachments = attachments.filter((attachment) => attachment.kind === 'image');
  if (imageAttachments.length && EDIT_PATTERNS.some((pattern) => pattern.test(text))) {
    return { intent: 'edit', confidence: 'high', reason: 'image-edit-with-attachment' };
  }
  if (HIGH_CONFIDENCE_GENERATE_PATTERNS.some((pattern) => pattern.test(text))) {
    return {
      intent: imageAttachments.length ? 'edit' : 'generate',
      confidence: 'high',
      reason: imageAttachments.length ? 'explicit-generate-with-image' : 'explicit-generate'
    };
  }
  if (imageAttachments.length && /(生成|出图|重绘|做成|变成|generate|create|draw|make)/i.test(text)) {
    return { intent: 'edit', confidence: 'high', reason: 'image-attachment-transform' };
  }
  if (MEDIUM_CONFIDENCE_GENERATE_PATTERNS.some((pattern) => pattern.test(text))) {
    return {
      intent: imageAttachments.length ? 'edit' : 'generate',
      confidence: 'medium',
      reason: 'ambiguous-visual-design'
    };
  }
  if (GENERATE_PATTERNS.some((pattern) => pattern.test(text))) {
    return {
      intent: imageAttachments.length ? 'edit' : 'generate',
      confidence: 'medium',
      reason: 'legacy-image-keyword'
    };
  }
  return { intent: null, confidence: 'none', reason: 'no-image-intent' };
}

function detectImageIntent(message, attachments) {
  return analyzeImageIntent(message, attachments).intent;
}

export function isImageRequest(message, attachments = []) {
  const analysis = analyzeImageIntent(message, attachments);
  return Boolean(analysis.intent && analysis.confidence === 'high');
}
