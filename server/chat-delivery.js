...
export async function assertDesktopBridgeAvailable(getDesktopBridgeStatus) {
  ...
  if (bridge && bridge.strict && !bridge.connected) {
...
function isDesktopFollowerPreflightTimeout(error) {
  if (error?.code !== 'CODEXMOBILE_DESKTOP_IPC_TIMEOUT') return false;
...
function userMessageMetadataForSendMode(sendMode = 'start') {
  return sendMode === 'steer'
    ? { guided: true, guideLabel: '已引导对话', kind: 'guided_user' }
    : {};
}
...
async function syncDesktopFollowerCollaborationMode({
  ...
  if (!setDesktopFollowerCollaborationMode) return;
...
export async function sendViaDesktopIpc({
  ...
  const input = buildCodexTurnInput({ message: codexMessage, attachments, selectedSkills });
  ...
  async function attemptDesktopFollowerTurn() {
    if (sendMode === 'steer') {
      ...
      if (sendMode === 'interrupt') await interruptDesktopFollowerTurn(selectedSessionId);
      if (setDesktopFollowerModelAndReasoning) {
        await setDesktopFollowerModelAndReasoning(selectedSessionId, model || null, reasoningEffort || null);
      }
      await syncDesktopFollowerCollaborationMode({
  ...
  try {
    for (let attempt = 0; ; attempt += 1) {
      try {
        ...
        if (!isDesktopThreadOwnerUnavailable(error) || attempt >= ownerRetryDelays.length) throw error;
...
export function runQueuedHeadlessChatJob({
  ...
  function rememberStartedBackgroundThread(payload) {
    if (!payload?.sessionId || !job.draftSessionId) return;
    registeredSessionIds.add(payload.sessionId);
  ...
  runCodexTurn(
    ...
    (payload) => {
      const eventPayload = { ...payload, source: payload?.source || 'headless-local' };
      ...
      if (eventPayload.type === 'thread-started' || eventPayload.type === 'chat-started') {
        rememberStartedBackgroundThread(eventPayload);
      }
      if (eventPayload.type === 'chat-error' && registeredSessionIds.size) {
        // The first turn died — the session file was likely never written, so
        // the eager registration above would linger as a phantom drawer row
        // carrying the user's message. Withdraw it; a session that DID reach
        // disk simply loses a redundant title override.
        const ids = [...registeredSessionIds];
        registeredSessionIds.clear();
        metadataUpdates.push(
          unregisterMobileSessions(ids).catch((error) => {
            console.warn('[sessions] Failed to unregister failed thread:', error.message);
          })
        );
      }
    ...
    try {
      if (metadataUpdates.length) await Promise.allSettled(metadataUpdates);
      ...
      state.running = false;
      if (state.jobs.length) onQueueDrained?.();
    }
  });
}
